...
function ChartTooltipContent({
  ...
  active?: boolean
  payload?: Array<{ name?: string | number; value?: number | string; dataKey?: string | number; color?: string; fill?: string; payload?: Record<string, unknown> }>
  label?: string | number
  ...
  return (
    <div className={cn('border-transparent bg-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border-transparent px-2.5 py-1.5 text-xs shadow-xl', className)}>
  ...
  return (
    <div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map((item) => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key)
        return (
          <div key={String(item.value ?? item.dataKey)} className="[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3">
