...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback(() => (isMobile ? setOpenMobile((open) => !open) : setOpen((open) => !open)), [isMobile, setOpen])
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<'div'> & { showIcon?: boolean }) {
  ...
  const width = React.useMemo(() => {
    ...
    const stableIndex = Array.from(skeletonId).reduce((sum, character) => sum + character.charCodeAt(0), 0)
